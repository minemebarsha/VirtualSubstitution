...
\begin{frame}{Motivation}
	\begin{itemize}
		...
		\item Virtual substitution
			\begin{itemize}
				\item applicable only to sub-language
...
\begin{frame}{Real Arithmetic Formula}
	\begin{itemize}
		...
		{\only<2>{\color{blue}}\textbf{terms:}\hspace{12mm}$t\hspace{0mm}:=\hspace{3mm}0\hspace{3mm}|\hspace{3mm}1\hspace{3mm}|\hspace{3mm}x\hspace{3mm}|\hspace{3mm}t+t\hspace{3mm}|\hspace{3mm}t - t\hspace{3mm}|\hspace{3mm}t \cdot t$\newline}
		{\only<3>{\color{blue}}\textbf{constraints:}\hspace{0mm} $c\hspace{0mm}:=\hspace{2mm}t<t$\newline}
		{\only<4>{\color{blue}}\textbf{formulas:}\hspace{4mm} $\varphi\hspace{0mm}:=\hspace{3mm}c\hspace{3mm}|\hspace{3mm}\neg\varphi\hspace{3mm}|\hspace{3mm}\varphi\wedge\varphi\hspace{3mm}|\hspace{3mm}\exists x\cdot\varphi$\newline}
		...
		Example:
		$$\varphi = (\underbrace{({\only<6>{\color{blue}}x^{2}}+{\only<7>{\color{blue}}2}{\only<6>{\color{blue}}x}+{\only<7>{\color{blue}}4}{\only<7>{\color{blue}}z})}\limits_{{\only<8>{\color{blue}}p_{1}}}\leq 0\vee \underbrace{(y{\only<6>{\color{blue}}x^{2}}+{\only<7>{\color{blue}}6y^{3}}{\only<6>{\color<6>{blue}}x}+{\only<7>{\color{blue}}4}{\only<7>{\color{blue}}z})}\limits_{{\only<8>{\color{blue}}p_{2}}}= 0)$$
	\end{itemize}
\end{frame}
\begin{frame}{Flow Chart of Virtual Substitution}
	\begin{center}
		\includegraphics[scale=0.28]{FlowChart-3.pdf}
	\end{center}
\end{frame}
...
\begin{frame}{Compute Zeros}
	...
	\begin{center}
		\input{qzeros.tex}
	\end{center}
	$\xi_{1} = \frac{-b - \sqrt{b^{2}-4ac}}{2a}$, $\xi_{2} = \frac{-b + \sqrt{b^{2}-4ac}}{2a}$\newline
}
\end{frame}
...
\begin{frame}{Example}
	...
	\begin{table}[]
		...
		\end{tabular}
	\end{table}}
\only<2>{\includegraphics[scale=0.28]{FlowChart-1.pdf}}
	\only<3->{
		\begin{table}[]
			\centering
...
\begin{frame}{Substitution of Square Root Expression}
	...
	\visible<3->{
		...
		\visible<8->{$\Longleftrightarrow u^{\prime}q^{\prime}\leq 0 \text{ } \wedge \text{ } {u^{\prime}}^{2} - {q^{\prime}}^{2}\sqrt{r} = 0$}
	}
	\end{enumerate}
\end{frame}
